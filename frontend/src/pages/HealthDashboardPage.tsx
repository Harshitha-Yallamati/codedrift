...
import { formatPercent, formatRelativeDate } from "@/lib/risk";
...
export function HealthDashboardPage() {
  ...
  const { data: health, isLoading } = useQuery({ queryKey: ["health", id], queryFn: () => dashboardApi.health(id) });
          ...
          <div className="flex items-center">
            ...
            <div className="ml-2 space-y-2">
              {(["critical", "high", "medium", "low"] as const).map((cat) => (
                <div key={cat} className="flex items-center justify-between gap-6 text-sm">
              ...
              >
                ...
                {f.risk_prediction && (
                  <span className="mt-1 block text-[11px] text-slate-500">
                    {formatPercent(f.risk_prediction.defect_probability)} predicted defect probability
                  </span>
                )}
              </Link>
            ))}
